 ...
 * common in WebView runtimes that have no speech backend): record with
 * MediaRecorder and transcribe via the active profile's Hermes Gateway STT
 * (`/api/audio/transcribe`) through the Tauri backend.
 *
 * Some STT providers (e.g. Groq) have no streaming ASR over HTTP, so to keep
 * the recorder path *live* we re-transcribe the growing recording every
 * {@link LIVE_INTERVAL_MS} and push the running transcript out as an interim
 * result; on stop we do a final pass over the whole clip. It re-sends the audio
 * so far on each tick, but voice input is short and Whisper-class STT is fast.
 *
...
export function useVoiceInput(
  ...
  const transcribeAccumulated = useCallback(
    async (isFinal: boolean): Promise<void> => {
      ...
      try {
        ...
        const text = await transcribeHermesAudio(bytes, blob.type, profile);
  ...
  const startSpeechRecognition = useCallback(() => {
    ...
    rec.onerror = (event) => {
      ...
      setRecording(false);
      // A WebView/Chromium without a reachable speech backend → fall back to
